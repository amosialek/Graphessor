...
namespace Rivara{
    ...
    TEST_F(RivaraProduction3Test, P3TestNewENodeLAttribute)
    {
        this->E2.attributes->SetBool(RIVARA_ATTRIBUTE_B, true);
        Pixel newNNode = this->p3->GetNewNNode(this->nodes);
        Pixel p = this->p3->GetNewENode(newNNode, this->nodes);
        ASSERT_DOUBLE_EQ(p.attributes -> GetDouble(RIVARA_ATTRIBUTE_L), 2.5);
    }

    TEST_F(RivaraProduction3Test, P3TestNewENodeBAttributeTrue)
    {
        this->E2.attributes->SetBool(RIVARA_ATTRIBUTE_B, true);
        Pixel newNNode = this->p3->GetNewNNode(this->nodes);
        Pixel p = this->p3->GetNewENode(newNNode, this->nodes);
    ...
    TEST_F(RivaraProduction3Test, P3TestNewENodeBAttributeFalse)
    {
        this->E2.attributes->SetBool(RIVARA_ATTRIBUTE_B, false);
        Pixel newNNode = this->p3->GetNewNNode(this->nodes);
        Pixel p = this->p3->GetNewENode(newNNode, this->nodes);
        ASSERT_EQ(p.attributes -> GetBool(RIVARA_ATTRIBUTE_B), false);
    }

    TEST_F(RivaraProduction3Test, P3TestNewNNode)
    {
        Pixel p = this->p3->GetNewNNode(this->nodes);
    ...
    TEST_F(RivaraProduction3Test, P3TestNewEMiddleNodeBAttribute)
    {
        Pixel newNNode = this->p3->GetNewNNode(this->nodes);
        Pixel p = this->p3->GetNewEMiddleNode(newNNode, node3Vertex);
        ASSERT_EQ(p.attributes -> GetBool(RIVARA_ATTRIBUTE_B), false);
    }

    TEST_F(RivaraProduction3Test, P3TestNewEMiddleNodeLAttribute)
    {
        Pixel newNNode = this->p3->GetNewNNode(this->nodes);
        Pixel p = this->p3->GetNewEMiddleNode(newNNode, node3Vertex);
    ...
    TEST_F(RivaraProduction3Test, P3TestNewTNode)
    {
        auto TNode = p3 -> GetNewTNode();
